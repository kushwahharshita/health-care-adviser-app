...
public class ChatListCtl extends BaseCtl {
	/**
	 * Logger to log the messages.
	 */

	private static Logger log = Logger.getLogger(ChatListCtl.class);
	@Override
	...
	protected void doGet(HttpServletRequest request,
		...
		log.debug("ChatListCtl doGet Start");
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op)
				...
				ServletUtility.redirect(ORSView.CHAT_CTL, request, response);
				return;
			} else if (OP_DELETE.equalsIgnoreCase(op)) {
				pageNo = 1;
				if (ids != null && ids.length > 0) {
					ChatModel deletemodel = new ChatModel();
					for (String id : ids) {
			...
			ServletUtility
					.forwardView(ORSView.CHAT_LIST_VIEW, request, response);
		...
		log.debug("ChatListCtl doGet End");
	}

	@Override
	protected String getView() {
		return ORSView.CHAT_LIST_VIEW;
	}
